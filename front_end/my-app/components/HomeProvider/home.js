...
export default function Home({ navigation }) {
  ...
  const [searchMethod, setSearchMethod] = useState('city'); // Méthode de recherche par défaut
  ...
  const fetchBookings = async () => {
    try {
      const response = await axios.get('http://192.168.1.3:5003/api/bookings/GetAll');
    ...
    try {
      ...
      if (searchInput.trim()) {
        if (searchMethod === 'city') {
          response = await axios.get(`http://192.168.1.3:5003/api/bookings/GetByCity/${searchInput}`);
        } else if (searchMethod === 'category') {
          response = await axios.get(`http://192.168.1.3:5003/api/bookings/GetByCategory/${searchInput}`);
        }
      } else {
        response = await axios.get('http://192.168.1.3:5003/api/bookings/GetAll');
      }
      setBookings(response.data);
    } catch (error) {
      console.error('Error fetching bookings:', error);
