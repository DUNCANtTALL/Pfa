...
export default function BookingPage({ route, navigation }) {
    ...
    const handleCompleteBooking = async (id) => {
        try {
            await axios.put(`http://192.168.1.3:5003/api/bookings/Complete/${id}`);
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            if (client) {
                try {
                    const response = await axios.get(`http://192.168.1.3:5003/api/bookings/bookings/user/${client}`);
    ...
    useEffect(() => {
        const fetchProviders = async () => {
            ...
            for (const id of uniqueProviderIds) {
                try {
                    const response = await axios.get(`http://192.168.1.3:5003/api/users/getByID/${id}`);
